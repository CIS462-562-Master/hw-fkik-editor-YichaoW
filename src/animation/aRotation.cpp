...
void quat::FromRotation(const mat3& rot)
{
	...
	if (trace > 0) {
		...
		mQ[VZ] = (rot[2][1] + rot[1][2]) / (4 * mQ[VY]);
	}
	else {
		mQ[VZ] = sqrt(1.0 + rot[2][2] - rot[0][0] - rot[1][1]) / 2.0;
		mQ[VW] = (rot[1][0] - rot[0][1]) / (4 * mQ[VZ]);
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
	...
	quat tempQ = q1;
	double dotQs = Dot(q0, q1);
	if (dotQs < 0) {
		dotQs = -dotQs;
		tempQ = -tempQ;
	}
	double omega = acos(dotQs);

	q = q0 * sin((1 - u) * omega) / sin(omega) + tempQ * sin(omega * u) / sin(omega);
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
	...
	double divisor = sin(angleRad / 2);
	if (divisor < EPSILON) {
		divisor = 1;
	}
	axis[0] = mQ[VX] / divisor;
	axis[1] = mQ[VY] / divisor;
	axis[2] = mQ[VZ] / divisor;
}
